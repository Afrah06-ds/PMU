...
/**
 * Shared core PDF extraction logic that works on both:
 * 1. Server-side (Node.js API route)
 * 2. Client-side (Browser with PDF.js)
 */
export async function parsePdfDocument(doc: any): Promise<{
  numPages: number;
  ...
  if (numPages >= 1) {
    ...
    for (const r of leftRows) {
      ...
      const codeIt = r.items.find(
        (i) =>
          /^[A-Z][A-Z0-9]{5,8}$/.test(i.str.trim()) &&
          !['PERIYAR', 'SCIENCE', 'FACULTY', 'PROGRAMME'].includes(i.str.trim())
      );
      if (codeIt) {
