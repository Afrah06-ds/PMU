...
import { BookOpen, Building2, CalendarDays, Check, CheckCircle2, Clock3, FileText, GripVertical, Plus, Sparkles, Trash2, WandSparkles } from 'lucide-react';
...
export default function GeneratePaperPage() {
  ...
  useEffect(() => {
    Promise.all([MasterDataService.getDepartments(), MasterDataService.getKLevels()]).then(([departmentList, levelList]) => {
      setDepartments(departmentList);
      setKlevels(levelList);
      if (departmentList[0]) setDepartmentId(departmentList[0].id);
    });
  }, []);
  ...
  useEffect(() => {
    if (!courseId) return;
    Promise.all([MasterDataService.getModules(courseId), MasterDataService.getCourseOutcomes(courseId)]).then(([moduleList, outcomeList]) => {
  ...
  const generate = async () => {
    ...
    const result = await QuestionGeneratorEngine.generatePaper({ department_id: departmentId, course_id: courseId, college_name: collegeName, exam_name: `${examNameLine1} - ${examNameLine2}`, exam_name_line1: examNameLine1, exam_name_line2: examNameLine2, target_branch_class: targetBranchClass, semester, academic_year: academicYear, date_of_exam: dateOfExam, duration_minutes: durationMinutes, sections });
    setLoadingStep(5);
    await new Promise((resolve) => setTimeout(resolve, 300));
